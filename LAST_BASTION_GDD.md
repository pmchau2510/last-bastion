# LAST BASTION — Game Design Document v6.0
...
| # | Tên | Độ khó | Kích thước | Số làn | Thời tiết | Đặc điểm |
|---|-----|--------|-----------|--------|-----------|----------|
| 0 | Ironhold Pass | ⭐⭐ | 36×24 | 2 | Ngẫu nhiên | Hẻm núi, đường zíc-zắc, map dễ nhất |
| 1 | Sylvan Crossing | ⭐⭐⭐ | 40×28 | 2 | Mưa/Bão tuyết | Rừng che, 2 làn cuộn vào nhau |
| 2 | Tidal Docks | ⭐⭐⭐ | 42×28 | 2 | Giông/Nắng | 2 làn song song, nhiều đoạn cong |
| 3 | Ashfield Ruins | ⭐⭐⭐⭐ | 44×30 | 2 | Nắng cố định | Đường ngoằn ngoèo dài, 2 làn |
| 4 | Crimson Rift | ⭐⭐⭐⭐ | 46×30 | 3 | Nhật thực định kỳ | 3 làn đổ về cùng lúc — cần tổng lực team |
| 5 | Void Nexus | ⭐⭐⭐⭐⭐ | 48×32 | 2 | Chaos | 🔒 Mở sau khi thắng 5 map, tối hoàn toàn |

**Cơ chế multi-path:**
- `pathFns: [fn1, fn2, ...]` — mảng các hàm tạo path theo chiều rộng màn hình.
- `resize()` tính Y-range chung cho tất cả paths → scale đồng đều.
- Mỗi enemy nhận `pathIdx = g % numPaths` khi spawn; di chuyển trên đúng path của mình.
...
### 8.2 Boss (xuất hiện round 5, 10, 15, 20)
...
### 8.3 Sự kiện đặc biệt theo round
...
### 8.4 Khoảng cách spawn

Quái ra **liên tục** từng con, không chờ con trước đến trước:

| Round | Delay (frames @60fps) | Xấp xỉ |
|-------|----------------------|---------|
| 1 | 90 | ~1.5s |
| 2 | 70 | ~1.2s |
| 3 | 55 | ~0.9s |
...
Delay trên là delay giữa **từng con liên tiếp** trong spawnQueue. Cổng 2 trở đi stagger thêm `30 + gateIdx×15` frames.
...
```
[Wave Announce: "ROUND X"] → [Giai đoạn PREP] → [▶ Bắt đầu / Đếm ngược hết] → [Giai đoạn WAVE] → [Quái hết + không còn ai trên sân] → [Thưởng + Round tiếp]
```
...
- **Thắng:** Sống sót qua Round 20 (Standard/Challenge).
- **Thua:** HP Tinh Thể về 0 (mỗi quái lọt qua trừ HP theo hp\_value).
...
```
[Round X/20] [⏱/⚔️ phase]   [● Gold] [💎 HP]   [▶ Bắt đầu] [⏸] [🔊]
[Quota bar ─────────────────────────────────── 0/5]
...
- Mỗi quái lọt qua cổng cuối → đánh dấu 1 ô quota.
- Ô quota đầy → **Game Over**.
- Tổng quota = theo map và mode (Standard = 5, Hardcore × 0.4 ≈ 2).
- HUD quota hiển thị ô màu: xanh (ok), đỏ (đã leak), xám (còn trống).
...
### 14.2 Admin password để tạo phòng

- Nút "Tạo phòng mới" ẩn mặc định.
- Nhập mật khẩu admin vào ô → bấm 🔓 → nếu đúng (`BASTION`) thì nút hiện ra.
- Guests chỉ cần nhập mã phòng 6 ký tự, không cần mật khẩu.
- Host có toàn quyền chọn map/mode trong lobby.

### 14.3 Lobby — Điều kiện bắt đầu

- Host chọn map/mode; guests chọn hero và **bắt buộc bấm "✓ Sẵn sàng"**.
- Nút "🚀 Bắt đầu" chỉ active khi **tất cả guest đã sẵn sàng**.
- Counter: `⏳ Chờ guests sẵn sàng (0/N)` → `🚀 Bắt đầu (N người)`.

### 14.4 Chọn Nation trong Lobby
...
### 14.5 State snapshot (`state_sync`)
...
### 14.6 Guest input relay (`player_input`)
...
### 14.8 Reconnect sau khi mất kết nối
...
### 14.9 Nút "▶ Bắt đầu" trong multiplayer
...
Menu
 ...
 └─ [👥 Nhiều người chơi] → MP Overlay → Lobby (chọn Nation + Ready) → Game
